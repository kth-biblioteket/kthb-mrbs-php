...
//191003 flyttat user, is_admin.
// Also need to know whether they have admin rights
$user = getUserName();
$is_admin = (authGetUserLevel($user) >= 2);

//Original som inte används....
...
for ($i = 0; ($row = sql_row_keyed($res, $i)); $i++)
{
  ...
  } elseif($row['entry_field_name'] == 'end_time'){
    $edit_entry_field_order[] = 'end_date';
  } else {
    $edit_entry_field_order[] = $row['entry_field_name'];
  }
}
//lägg till nödvändiga placeholders för fält
...
{
  global $name, $maxlength, $is_mandatory_field, $is_admin, $create_by;
  ...
  echo "</div>\n";
  //Visa bokarens ID för administratörer
  if($is_admin) {
    /*
    $currentuser 			= getuser($create_by);
    ...
    */
    //echo '<div><span style="width:10em;text-align: left;margin: 0;font-weight: bold;float: left;padding-bottom: 1em;">Bokad av: </span><span style="width: auto;margin-right: 1em;float: left;font-weight: bold;">' .  $create_by . '</span></div>';
  }
}
...
{
  global $start_time, $areas, $area_id, $periods, $id, $is_admin;;
  //KTH
  //om period så hämta startdatum för sloten från period-tabell i databasen.
  ...
  //KTH visa inte för icke admin och flytta att ligga mellan slot och all day
  //191003 visa inte datumväljare för någon
  if ($is_admin) {
	echo "<div style=\"visibility: hidden; height: 0px; float: left;\">\n"; // Needed so that the structure is the same as for the end date to help the JavaScript
  } else {
	echo "<div style=\"visibility: hidden; height: 0px; float: left;\">\n"; // Needed so that the structure is the same as for the end date to help the JavaScript
  }
  ...
  echo "</div>\n";
  //SLUT KTH
  //191003 Visa inte all day för någon
  genAllDay($areas[$area_id], 'all_day', 'all_day', TRUE, $disabled);
...
function create_field_entry_end_date($disabled=FALSE)
{
  ...
  //KTH flytta date att ligga efter tiden
...
{
  global $areas, $area_id, $rooms, $select_area_allowed;
  ...
  if (count($areas) > 1)
  {
	...
	//if ($select_area_allowed) {
		//echo "<div id=\"div_areas\" style=\"display: none\">\n";
	//} else {
		echo "<div id=\"div_areas\" style=\"display: none; visibility:hidden; height: 0px;\">\n";
	//}
    $options = array();
...
function create_field_entry_rooms($disabled=FALSE)
{
  ...
  //} else {
    echo "<div id=\"div_rooms_kth\" style=\"visibility:hidden; height: 0px;\">\n";
    echo "<label for=\"rooms_kth\" style=\"border:none;visibility:hidden; height: 0px;\">" . get_vocab("room")  . ":</label>\n";
    echo "<input id=\"rooms_kth\" style=\"border:none;visibility:hidden; height: 0px;\" value=\"" . get_room_name($room_id,$lang) . "\"></input>";
    echo "</div>\n";
    echo "<div id=\"div_rooms\" style=\"position:fixed;visibility:hidden; height: 0px;\">\n";
  //}
  //echo "<div id=\"div_rooms\">\n";
...
{
  global $booking_types, $type, $is_admin;
  //KTH
  if ($is_admin) {
	  echo "<div style=\"display:block\" id=\"div_type\">\n";
  } else {
	  echo "<div id=\"div_type\">\n";
  }

  //191003 om det är en bokningsbar tid "B" och en vanlig user, sätt bokningstyp till default
  if($type == 'B' && !$is_admin){
    $type = 'I';
  } else {
...
function create_field_entry_confirmation_status($isbooking,$disabled=FALSE)
{
	...
	if ($confirmation_enabled)
	{
		...
		if ($enable_periods) {
			$period_slot_starttime = get_period_slot_starttime($area,1 * date("i",$start_time));
			$bookingdate = date("Y-m-d",$start_time) . " " . $period_slot_starttime;
		} else {
			$bookingdate = date("Y-m-d H:i:s",$start_time);
		}

		$bookingdateplus60seconds = date("Y-m-d H:i:s",strtotime($bookingdate. " +60 minutes"));
		$confirmtimestart = date("Y-m-d H:i:s",strtotime($bookingdate. " -" . CONFIRM_MINUTES ." minutes"));
		$confirmtimeend = date("Y-m-d H:i:s",strtotime($bookingdate. " +" . CONFIRM_MINUTES ." minutes"));

		//Om det är en adminbokning eller statusen redan är bekräftad så ska den alltid bekräftas!
		if ($is_admin) {
			...
			//om det är en befintlig bokning och tiden ligger i intervall för confirm ska confirm visas
			if ($today >= $confirmtimestart && $today <= $confirmtimeend && $isbooking == TRUE) {
				//echo "om det är en befintlig bokning och tiden ligger i intervall för confirm ska confirm visas";
				$confirm_kth = 1;
				echo "<div id=\"div_confirmation_status\" style=\"display:none\">\n";
				$buttons[1] = get_vocab("confirmed");
				$params = array('label'       => get_vocab("confirmation_status") . ":",
								'name'        => 'confirmed',
								//'value'       => ($confirmed) ? 1 : 0,
				...
				if ($today >= $confirmtimestart && $today <= $confirmtimeend && $isbooking != TRUE) {
					//echo "om det är en NY bokning och tiden ligger i intervall för confirm ska confirm visas";
					echo "<div id=\"div_confirmation_status\" style=\"display:none\">\n";
					$buttons[1] = get_vocab("confirmed");
					$params = array('label'       => get_vocab("confirmation_status") . ":",
								'name'        => 'confirmed',
					...
					// om nutid är före starttiden för bokningen ska Tentative visas
					if($today < $bookingdate) {
						//echo "om nutid är före starttiden och om intervall för confirm inte är aktuellt för bokningen ska Tentative visas";
						$confirm_kth = 4;
						echo "<div id=\"div_confirmation_status\" style=\"display:none\">\n";
						...
						//om nutid är efter starttiden för bokningen  ska confirm visas
						//if ($today <= $bookingdateplus60seconds) {
							//echo "om nutid är efter starttiden för bokningen och om inte en timme passerat ska confirm visas";
							echo "<div id=\"div_confirmation_status\" style=\"display:none\">\n";
							$buttons[1] = get_vocab("confirmed");
							$params = array('label'       => get_vocab("confirmation_status") . ":",
											'name'        => 'confirmed',
			...
			//KTH Visa status
			if ($confirm_kth == 0) {
				echo "<div id=\"div_confirmation_status_kth\">\n";
				echo "<label>" . get_vocab("confirmation_status") . ":&nbsp</label>\n";
				echo "<span>" . get_vocab("confirmed") . "</span>";
				echo "</div>\n";
			} 
			if ($confirm_kth == 1 && $confirmed != 1) {
				echo "<div id=\"div_confirmation_status_kth\">\n";
				echo "<label>" . get_vocab("confirmation_status") . ":&nbsp</label>\n";
				echo "<span>" . get_vocab("to_be_confirmed") . "</span>";
				echo "</div>\n";
			} 
			if ($confirm_kth == 4) {
				echo "<div id=\"div_confirmation_status_kth\">\n";
					echo "<label>" . get_vocab("confirmation_status") . ":&nbsp</label>\n";
					echo "<span>" . get_vocab("tentative") . "</span>";
				echo "</div>\n";
				echo "<div id=\"editentryinfo\" style=\"padding: 20px 0px;\">\n";
					echo "<img style=\"width: 70px;height:70px;float: left;\" src=\"images/utropstecken2.png\">\n";
					echo "<div style=\"float: left;padding-left: 10px;width: 250px;\">\n";
						echo "<b>" . get_vocab("remembertoconfirm") . "</b>";
					echo "</div>\n";
					//echo "<img style=\"width: 50px;height:70px;float: left;\" src=\"images/utropstecken2.png\">\n";
				echo "</div>\n";
			}
		}
	}
    ...
    {//TODO hårdkodning bort.
      ...
      case 'campus':
        ...
        case 'kth_assignment':
          ...
          $places = array();
          //Endast de som inte är disabled
          $sql = "SELECT kth_assignments.id, kth_assignments.name, kth_assignments.name_en
                  FROM kth_assignments
                  WHERE kth_assignments.disabled = 0
                  ORDER BY kth_assignments.sort_key";
          ...
          //Det ska finnas ett entry i kth_assignments som är "1none" som agerar dummy för att ha ett "select an option"
          for ($i = 0; ($row = sql_row_keyed($res, $i)); $i++)
          {
            $places[] = $row['id'];
            if ($row['name'] == '1none') {
      ...
      default:
        ...
        $fieldvalue = NULL;
        //191003
        //Sätt värden för icke admin(från Alma/LDAP via inloggningen)
        $customfieldfromuser = false;
        if (!$is_admin) {
          if ($key == 'phone') {
            $fieldvalue = getPhone();
            $customfieldfromuser = true;
            $disabled = true;
          }
          if ($key == 'email') {
            $fieldvalue = getEmail();
...
//KTH
$select_area_allowed = $is_admin || empty($auth['only_admin_can_select_area']);

//öppettider bara för admins
//TODO Hårdkodning bort
if ($area=="10" && !$is_admin) {
  ...
  foreach ($row as $column => $value)
  {
    switch ($column)
    {
      ...
      case 'create_by':
        ...
        //191003 Sätt alltid create_by till aktuell inloggad användare om det är en bokningsbar tid
        if ($is_admin){
          //$create_by = $user;
        }
        if ($row['type'] == 'B'){
...
else
{
  ...
  //191003 Sätt bokningstyp till bokningsbar för admins om bokningar inte ska vara öppna som default
  if ($is_admin && !$default_bookings_open) {
    $type          = 'B';
  } else {
    $type          = $default_type; //= "I"
...
//191003 Bokningsbara tider ska gå att boka av alla användare.
if (!getWritable($create_by, $user, $room_id) && $row['type'] != 'B')
{
  showAccessDenied($day, $month, $year, $area, isset($room) ? $room : "");
  exit;
}
...
<form class="form_general" id="main" action="edit_entry_handler.php" method="post">
  <fieldset>
  <?php//KTH visa areanamn(resursnamn?)?>
  <?php 
  //echo  get_area_name($area) . " - " . get_vocab($token) .  ", " . date("Y-m-d", $start_time) . "" ; 
  echo  "<h3>";
  if(get_room_number($room_id,$lang) !="") { 
	echo get_room_number($room_id,$lang) . ". ";
	}
  echo get_room_name($room_id,$lang) . " - " . get_vocab($token) .  ", " . date("Y-m-d", $start_time) . "</h3>" ; 
  ?>
...
foreach ($edit_entry_field_order as $key)
{
  ...
  case 'start_date':
  //KTH bara admin får ändra startdatum...
	if (!$is_admin) {
		create_field_entry_start_date(TRUE);
	} else {
		create_field_entry_start_date(FALSE);
	}
    break;

  case 'end_date':
    //KTH bara admin får ändra slutdatum...
    if (!$is_admin && !$default_bookings_open) {
      create_field_entry_end_date(TRUE);
    } else {
      create_field_entry_end_date(FALSE);
    }
    break;

  case 'areas':
    create_field_entry_areas(TRUE);
    break;

  case 'rooms':
    create_field_entry_rooms(TRUE);
    break;

  case 'type':
    //191003 visa inte bokningstyper för vanliga användare
    if (!$is_admin && !$default_bookings_open) {
      create_field_entry_type(TRUE);
    } else {
      create_field_entry_type(FALSE);
    }
    break;
  ...
  if (isset($id)){
    ...
    //KTH bara admin får ändra...
	if (!$is_admin) {
		create_field_entry_confirmation_status($isbooking,TRUE);
	} else {
		create_field_entry_confirmation_status($isbooking);
	}
    break;
  ...
  default:
  if ($extended_booking_form==1) {
    //echo "<h4>Vänligen fyll i: </h4>";
	  create_field_entry_custom_field($custom_fields_map[$key], $key, $is_admin, $user);
  }
  break;
  }
}
...
if (($edit_type == "series") && $repeats_allowed)
{
  ...
  echo "</fieldset>\n";
	...
	if ($id) {
		...
		{
		  //KTH
		  //echo "<a href=\"edit_entry.php?id=$id&amp;confirminterval=$confirminterval&amp;returl=$link_returl\">". get_vocab("editentry") ."</a>";
		} 
		...
		if ((!empty($repeat_id) || $series) && $repeats_allowed)
		{
		  echo "<a class= \"pinkbutton\" href=\"edit_entry.php?id=$id&amp;edit_type=series&amp;day=$day&amp;month=$month&amp;year=$year&amp;returl=$link_returl\">".get_vocab("editseries")."</a>";
		}
		echo "</div>\n";
      ...
      //visa bara om det inte är en "B" bookable
      if (($row['type'] != "B" && !$is_admin) || $is_admin) {
        echo "<a class= \"deletebutton btn btn-danger submit\" href=\"del_entry.php?id=$id&amp;series=0&amp;returl=$link_returl\" onclick=\"return confirm('".get_vocab("confirmdel")."');\">".get_vocab("deleteentry")."</a>";
      }
		}
		if (!empty($repeat_id) && !$series && $repeats_allowed)
		{
    ...
    echo "<div>\n";
      ...
      //visa bara om det inte är en "B" bookable
      if (($row['type'] != "B" && !$is_admin) || $is_admin) {
        echo "<a class= \"deletebutton btn btn-danger submit \" href=\"del_entry.php?id=$id&amp;series=1&amp;day=$day&amp;month=$month&amp;year=$year&amp;returl=$link_returl\" onClick=\"return confirm('".get_vocab("confirmdel")."');\">".get_vocab("deleteseries")."</a>";
      }
		}
		echo "</div>\n";
	}
	//KTH slut edit delete series
	
	//KTH flyttat att ligga ovanför knappar
