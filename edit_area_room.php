...
//KTH Timmar
function create_field_entry_max_hours_number()
{
  global $interval_types,
         $max_hours_per_interval_area_enabled, $max_hours_per_interval_global_enabled,
         $max_hours_per_interval_area, $max_hours_per_interval_global;
         
  // The max_per booking policies
  echo "<fieldset>\n";
  echo "<legend>" . get_vocab("hour_limits") . "</legend>\n";
  echo "<table>\n";
  ...
  {
    echo "<tr>\n";
    echo "<td><label>" . get_vocab("max_hours_per_${interval_type}") . ":</label></td>\n";
    echo "<td><input class=\"enabler checkbox\" type=\"checkbox\" id=\"area_max_hours_per_${interval_type}_enabled\" name=\"area_max_hours_per_${interval_type}_enabled\"" .
         (($max_hours_per_interval_area_enabled[$interval_type]) ? " checked=\"checked\"" : "") .
         ">\n";
    echo "<input class=\"text\" type=\"number\" min=\"0\" step=\"1\" name=\"area_max_hours_per_${interval_type}\" value=\"$max_hours_per_interval_area[$interval_type]\"></td>\n"; 
    echo "<td>\n";
    echo "<input class=\"checkbox\" type=\"checkbox\" disabled=\"disabled\"" .
         (($max_hours_per_interval_global_enabled[$interval_type]) ? " checked=\"checked\"" : "") .
         ">\n";
		 
    echo "<input class=\"text\" disabled=\"disabled\" value=\"" . $max_hours_per_interval_global[$interval_type] . "\">\n";
    echo "</td>\n";
    echo "</tr>\n";
  }
  echo "</tbody>\n";
...
function create_field_entry_max_number()
{
  ...
  foreach ($interval_types as $interval_type)
  {
    ...
    //echo "<input class=\"checkbox\" type=\"checkbox\" disabled=\"disabled\"" .
        ...
        //">\n";
    echo "<input class=\"enabler checkbox\" type=\"checkbox\" id=\"max_global_per_${interval_type}_enabled\" name=\"max_global_per_${interval_type}_enabled\"" . (($max_per_interval_global_enabled[$interval_type]) ? " checked=\"checked\"" : "") . ">\n";
    //echo "<input class=\"text\" disabled=\"disabled\" value=\"" . $max_per_interval_global[$interval_type] . "\">\n";
    echo "<input class=\"text\" type=\"number\" min=\"0\" step=\"1\" name=\"max_global_per_${interval_type}\" value=\"" . $max_per_interval_global[$interval_type] . "\">\n";
    echo "</td>\n";
    echo "</tr>\n";
  }
  echo "</tbody>\n";
...
//KTH Engelska
$area_name_en = get_form_var('area_name_en', 'string');
...
//KTH
$default_view = get_form_var('default_view', 'string');
$times_along_top = get_form_var('times_along_top', 'string');
$hide_links = get_form_var('hide_links', 'string');
...
//KTH
$custom_html_en = get_form_var('custom_html_en', 'string');  // Used for both area and room, but you only ever have one or the other
$reminder_email_enabled = get_form_var('reminder_email_enabled', 'string');
//KTH 2018
$extended_booking_form = get_form_var('extended_booking_form', 'string');
...
foreach ($interval_types as $interval_type)
{
  ...
  //KTH
  $var = "area_max_hours_per_${interval_type}";
  $$var = get_form_var($var, 'int');
  $var = "area_max_hours_per_${interval_type}_enabled";
  $$var = get_form_var($var, 'string');
  
  //KTH Globala regler
  $var = "max_global_per_${interval_type}";
  $$var = get_form_var($var, 'int');
  $var = "max_global_per_${interval_type}_enabled";
  $$var = get_form_var($var, 'string');
}
...
if ($phase == 2)
{
  ...
  { 
    ...
    foreach ($interval_types as $interval_type)
    {
	  ...
	  //KTH Timmar
	  $vars[] = "area_max_hours_per_${interval_type}_enabled";
	  
	  //KTH Globala regler
	  $vars[] = "max_global_per_${interval_type}_enabled";
    }
    foreach ($vars as $var)
    {
      $$var = (!empty($$var)) ? 1 : 0;
    ...
    if ((FALSE != $valid_email) && (FALSE != $valid_resolution) && (FALSE != $enough_slots))
    {
	  ...
	  //KTH Engelska
	  $assign_array[] = "area_name_en='" . sql_escape($area_name_en) . "'";
	  ...
	  //KTH
	  $assign_array[] = "custom_html_en='" . sql_escape($custom_html_en) . "'";
	  //kth nya areakolumner (default view, times_along_top, hide_links)
	  $assign_array[] = "default_view='" . sql_escape($default_view) . "'";
	  $assign_array[] = "times_along_top='" . sql_escape($times_along_top) . "'";
	  $assign_array[] = "hide_links='" . sql_escape($hide_links) . "'";
    $assign_array[] = "reminder_email_enabled='" . sql_escape($reminder_email_enabled) . "'";
    //KTH 2018
    ...
    $assign_array[] = "extra_info_en='" . sql_escape($extra_info_en) . "'";
      ...
      foreach($interval_types as $interval_type)
      {
		...
		//KTH Timmar
		$var = "max_hours_per_${interval_type}_enabled";
        $area_var = "area_" . $var;
        $assign_array[] = "$var=" . $$area_var;
        
        $var = "max_hours_per_${interval_type}";
        $area_var = "area_" . $var;
        if (isset($$area_var))
        {
          // only update these fields if they are set;  they might be NULL because
      ...
      $assign_array[] = "area_map=" . $area_map;
    ...
    <legend></legend>
      ...
      if ($is_admin)
      {
        ...
        echo "</div>\n";
		 // KTH english
        echo "<div>\n";
        $params = array('label'         => get_vocab("custom_html_en") . ":",
                        'label_title'   => get_vocab("custom_html_note"),
                        'name'          => 'custom_html_en',
...
if (isset($change_area) &&!empty($area))
{
  ...
  // KTH Area name engelska  
  echo "<div>\n";
  $params = array('label' => get_vocab("name_en") . ":",
                  'name'  => 'area_name_en',
                  'value' => $row['area_name_en']);
  generate_input($params);
  echo "</div>\n";
  ...
  //191003
  echo "<div>\n";
        $params = array('label'         => get_vocab("area_sort_key") . ":",
                        'label_title'   => get_vocab("area_sort_key_note"),
                        'name'          => 'area_sort_key',
                        'value'         => $row['area_sort_key'],
                        'disabled'      => $disabled,
                        'create_hidden' => FALSE);
        generate_input($params);
        echo "</div>\n";
  ...
  $options = array('1' => 'Grupprum',
                  ...
                  '3' => 'Handledning CAS',
                  '4' => 'Öppettider',
                  '5' => 'Talboksintroduktion',
                  '6' => 'Handledning KTH Biblioteket');
  $params = array('label'       => get_vocab("area_type") . ":",
                  'name'        => 'area_type',
                  'value'       => $row['area_type'],
                  'options'     => $options,
                  'force_assoc' => TRUE);
  ...
  */
  ...
  echo "<div id=\"defaultview\">\n";
  $options = array('0' => 'Day',
                   '1' => 'Week',
				   '2' => 'Month');
  $params = array('label'       => get_vocab("default_view") . ":",
                  'name'        => 'default_view',
                  'value'       => $row['default_view'],
                  'options'     => $options,
                  'force_assoc' => TRUE);
  generate_radio_group($params);
  echo "</div>\n";
  ...
  $options = array('0' => 'Side',
                   '1' => 'Top');
  $params = array('label'       => get_vocab("times_along_top") . ":",
                  'name'        => 'times_along_top',
                  'value'       => $row['times_along_top'],
                  'options'     => $options,
                  'force_assoc' => TRUE);
  generate_radio_group($params);
  echo "</div>\n";
  */
  ...
  $options = array('1' => 'Yes',
                   '0' => 'No');
  $params = array('label'       => get_vocab("hide_links") . ":",
                  'name'        => 'hide_links',
                  'value'       => $row['hide_links'],
                  'options'     => $options,
                  'force_assoc' => TRUE);
  generate_radio_group($params);
  echo "</div>\n";
  ...
  echo "<div id=\"reminder_email_enabled\">\n";
  $options = array('1' => 'Yes',
                   '0' => 'No');
  $params = array('label'       => get_vocab("reminder_email_enabled") . ":",
                  'name'        => 'reminder_email_enabled',
                  'value'       => $row['reminder_email_enabled'],
                  'options'     => $options,
                  'force_assoc' => TRUE);
  generate_radio_group($params);
  echo "</div>\n";
  ...
  //KTH 2018
  //extended_booking_form
  echo "<div id=\"extended_booking_form\">\n";
  $options = array('1' => 'Yes',
                  '0' => 'No');
  $params = array('label'       => get_vocab("extended_booking_form") . ":",
                  'name'        => 'extended_booking_form',
                  'value'       => $row['extended_booking_form'],
                  'options'     => $options,
                  'force_assoc' => TRUE);
  generate_radio_group($params);
  echo "</div>\n";
  ...
  //map
  echo "<div>\n";
  $params = array('label' => get_vocab("area_map") . ":",
                  'name'  => 'area_map',
                  'value' => $row['area_map']);
  generate_checkbox($params);
  echo "</div>\n";
  //mapimage
  echo "<div>\n";
  $params = array('label'       => get_vocab("area_map_image") . ":",
                  'name'        => 'area_map_image',
  ...
  create_field_entry_max_hours_number();
