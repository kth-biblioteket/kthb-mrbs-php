...
//KTH ?>
function updateperiods(area_id,query,id) {
	var data = {ajax: 1,
				commit: 1,
                starttime: $('#period_start_time').val(),
                endtime: $('#period_end_time').val(),
                areaid: area_id,
				query: query,
				id: id,
				uniq_param: (new Date()).getTime()};
	$.post('edit_area_room_handler.php',
            data,
            function(result){
                var div;
                if (result.length > 0)
...
<?php // Show or Hide the settings for Times and the note about Periods 
// as appropriate ?>
...
init = function() {
  ...
  //KTH
  var fixedColumnsOptions = {leftColumns: 0};
  makeDataTable('#periods_table', {paging: false,buttons:none}, fixedColumnsOptions);
};
