...
else
{
  ...
  get_area_settings($area);  // Update the area 
  //KTH
  unset($periods);
...
if (!getWritable($create_by, $user, $target_room) && $type != 'B')
{
...
//KTH logga inte ut på vanliga datorer
$returl .= "?year=$year&month=$month&day=$day&logout=false";
...
if (!empty($result['id']) && $result['valid_booking'])
{
      ...
      (!isset($result['id']) && $mail_settings['on_new']))
  {
    ...
    if($environment == 'development') {
      ...
      error_log($booking['reminder_email_enabled']);
    }
...
else
{
  ...
  //191003 kontrollera eventuell exchangebokning
  if (!empty($result['outlook']) && $result['outlook']['status'] != "OK")
  {
    echo "<p>\n";
    echo "Exchange error: " . ":\n";
    echo "</p>\n";
    echo "<ul>\n";
    echo "<li>" . $result['outlook']['message'] . "</li>\n";
    echo "</ul>\n";
  }
